...
export default function HomePage() {
  ...
  useEffect(() => {
    if (!apiBase) {
      setHealth('missing NEXT_PUBLIC_API_BASE_URL')
      return
    }
  ...
  const login = async (e: FormEvent) => {
    ...
    if (!res.ok) {
      setMessageType('err')
      return setMessage('Login failed. Check username/password.')
    }
  ...
  const createUser = async (e: FormEvent) => {
    ...
    if (!res.ok) {
      setMessageType('err')
      return setMessage('Could not create user (superadmin only)')
    }
    setNewUsername('')
    setNewUserEmail('')
    setNewUserPassword('')
  ...
  const addClient = async (e: FormEvent) => {
    ...
    if (!res.ok) {
      setMessageType('err')
      return setMessage('Could not create client')
    }
    setClientName('')
    setClientEmail('')
    await loadData()
  ...
  const addProject = async (e: FormEvent) => {
    ...
    if (!projectClientId) {
      setMessageType('err')
      return setMessage('Please select a client for the project')
    }
    if (!amountUsd || Number(amountUsd) <= 0) {
      setMessageType('err')
      return setMessage('Amount USD must be greater than 0')
    }
    if (!nextDate) {
      setMessageType('err')
      return setMessage('Next invoice date is required')
    }
    ...
    if (!res.ok) {
      setMessageType('err')
      return setMessage('Could not create project')
    }
    setProjectName('')
    setProjectClientId('')
    setAmountUsd('')
    setNextDate('')
    setRateType('tts_selling')
    await loadData()
  }
  ...
  const ingestPdfRate = async (e: FormEvent) => {
    ...
    if (!pdfUrl.startsWith('http://') && !pdfUrl.startsWith('https://')) {
      setMessageType('err')
      return setMessage('PDF URL must start with http:// or https://')
    }
    const res = await fetch(`${apiBase}/rates/ingest-pdf`, {
      method: 'POST',
      headers,
      body: JSON.stringify({ source_url: pdfUrl, target_code: 'USD' }),
    })
    if (!res.ok) {
      setMessageType('err')
      return setMessage('Could not ingest PDF rate')
    }
    setPdfUrl('')
    await loadData()
  }
  ...
  const runJobsNow = async () => {
    ...
    if (!res.ok) {
      setMessageType('err')
      return setMessage('Failed to run jobs')
    }
    await loadData()
  }
      ...
      {token && (
        <>
            ...
            <ul>
              {pagedUsers.map((u) => (
                <li key={u.id} style={{ marginBottom: 8 }}>
                  <b>{u.username}</b> ({u.email}) — role: {u.role} — status: {u.is_active ? 'active' : 'inactive'}{' '}
          ...
          <section style={{ marginTop: 24 }}>
            ...
            <h3>Clients</h3>
            <ul>
              {pagedClients.map((c) => <li key={c.id}>{c.name} ({c.email})</li>)}
            </ul>
            <p>
            ...
            <h3>Projects</h3>
            <ul>
              {pagedProjects.map((p) => <li key={p.id}>{p.name} - USD {p.amount_usd} - {p.next_invoice_date}</li>)}
            </ul>
            <p>
